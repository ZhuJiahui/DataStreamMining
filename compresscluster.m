...
% 压缩数据中中文时间戳对应关系 2013-01-01 0:00:00 = 41275
...
dictionary_filename = 'dataset/non_orthogonal/topics_data2/字典.txt';

write_directory1 = 'dataset/cluster/topics_data23/cluster_center';
write_directory2 = 'dataset/cluster/topics_data23/each_cluster_number';
write_directory3 = 'dataset/cluster/topics_data23/id_time';
write_directory4 = 'dataset/cluster/topics_data23/cluster_tag';
...
write_date_filename = 'dataset/cluster/topics_data23/date.txt';
batch_index_filename = 'dataset/cluster/topics_data23/batch_index.txt';
...
for i = 1 : length(data_files)
    ...
    elseif id_time{1, 2}(1) >= end_time_stamp
        ...
        while j <= candidate_j(end)
            ...
            if this_batch_id == previous_batch_id && (this_time_stamp - previous_time) < 2
                batch_data = [batch_data, pyramid_data(:, j)];
                %batch_id_time{end + 1} = [id_time{1, 1}(j), ' ', num2str(id_time{1, 2}(j))];
                % 里面每一项都是一个cell
                batch_id_time{end + 1} = strcat(id_time{1, 1}(j), char(127), num2str(id_time{1, 2}(j)));
                ...
                if j == candidate_j(end)
                    ...
                    if size(reconstruct_data, 2) <= ev_number
                        ...
                        write_idtime_to_text(strcat(strcat(write_directory3, '/'), strcat(num2str(file_number), '.txt')), batch_id_time);
                        dlmwrite(strcat(strcat(write_directory4, '/'), strcat(num2str(file_number), '.txt')), cluster_tag, ' ');
            ...
            %% while内第一层else %%
                ...
                if this_batch_id == previous_batch_id && (this_time_stamp - previous_time) >= 2
                    previous_time = previous_time + 2;
                elseif this_batch_id ~= previous_batch_id && (this_time_stamp - previous_time) < 2
